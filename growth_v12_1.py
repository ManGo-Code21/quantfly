...
logger = logging.getLogger("Growth_v12_1")
...
# 指数数据fallback
def get_index_data_robust(days=400):
    """获取上证指数日K线（push2his → 腾讯 fallback）"""
    df = get_index_data(days=days)
    ...
    try:
        ...
        if klines:
            ...
            logger.info(f"腾讯指数fallback: {len(df)}条")
            return df
...
def sentiment_score(df, idx):
    ...
    # 换手率飙升
    surge = 0.05 if turn_ratio > 2.0 else (0.02 if turn_ratio > 1.5 else 0)
    return surge


# ============================================================
# 动态选股（V12.1：增加质量+情绪因子，调整权重）
# ============================================================
def select_stocks_from_sectors(top_sectors, sector_stocks, stock_dfs, date, top_n=TOP_N):
    """在最强板块对应的股票池中选个股（V12.1增强版）"""
    stock_scores = []
    seen_codes = set()
    filtered_count = 0
...
# 主回测
# ============================================================
def growth_backtest_v12_1():
    logger.info("=" * 70)
    logger.info("V12.1 动态板块动量轮动 + 质量因子 + 情绪因子")
    logger.info("=" * 70)
    ...
    logger.info(f"\n{'='*70}")
    logger.info(f"V12.1 动态板块动量轮动 结果")
    logger.info(f"{'='*70}")
